...
# And for 31 days we have 31*24*3600 = 2678400
# So for 64 days we have 64*24*3600 =  5529600
scrubExpire=5529600
...
# Only email if there is a problem - RThickings @ 20171005
# but always send a telegram
telegram-send -g --pre "$(hostname) $emailMessage `/sbin/zpool list` `/sbin/zpool status`"
if [ "$problems" -ne 0 ]; then
  echo -e "$emailMessage \n\n\n `/sbin/zpool list` \n\n\n `/sbin/zpool status`" | mail -s "$emailSubject" support@dynacomitsupport.co.uk mrabee@laltexlondon.com
  logger $emailSubject
fi
